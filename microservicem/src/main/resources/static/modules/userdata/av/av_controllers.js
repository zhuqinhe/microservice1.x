...
.controller('avCtrl', ['$rootScope','$scope','sharedProperties','$http','$log','loginService', '$routeParams','commonMethod','$q','$location', function($rootScope,$scope,sharedProperties,$http,$log,loginService,$routeParams,commonMethod,$q,$location) {
    ...
    $scope.getPageData = function() {
        ...
        $scope.url = sharedProperties.getServerUrl()+"/ui/v1/review/user/av/list?contentId="+encodeURIComponent($scope.queryOptions.id)+"&name="+encodeURIComponent($scope.queryOptions.name)+"&begin="+($scope.pagingOptions.currentPage-1)*$scope.pagingOptions.pageSize+"&pageSize="+$scope.pagingOptions.pageSize;
