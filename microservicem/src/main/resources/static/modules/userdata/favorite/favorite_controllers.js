...
.controller('favoriteCtrl', ['$rootScope','$scope','sharedProperties','$http','$log','loginService', '$routeParams','commonMethod','$q','$location', function($rootScope,$scope,sharedProperties,$http,$log,loginService,$routeParams,commonMethod,$q,$location) {
    ...
    $scope.getPageData = function() {
        ...
        $scope.url = sharedProperties.getServerUrl()+"/ui/v1/favorite/list?userId="+encodeURIComponent($scope.queryOptions.id)+"&begin="+($scope.pagingOptions.currentPage-1)*$scope.pagingOptions.pageSize+"&pageSize="+$scope.pagingOptions.pageSize;
    ...
    $scope.dodelete = function(deleteFlag) {
        ...
        deleteDialog.then(function(value) {
            ...
            var appDeleteUrl=sharedProperties.getServerUrl()+"/ui/v1/favorite/delete";//delete的restful请求
